...
function initMouseInteractions() {
  ...
  document.addEventListener("contextmenu", function (e) {
    ...
    if (type === "voucher") {
      ...
      menuHTML = `
        <button class="context-menu-item" onclick="openEditTemplateModal('${escapedId}')">
          <svg fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" style="width:14px;height:14px;"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
          Chỉnh sửa phiếu mẫu
        </button>
        <button class="context-menu-item" onclick="modifySalesTemplate('${escapedId}')">
          <svg fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" style="width:14px;height:14px; color: var(--color-success);"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"></path></svg>
          Lên đơn từ mẫu này
        </button>
        <div class="context-menu-divider"></div>
        <button class="context-menu-item item-danger" onclick="deleteSalesTemplate('${escapedId}')">
          <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="width:14px;height:14px;"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
          Xóa phiếu mẫu này
        </button>
      `;
  ...
  document.addEventListener("keydown", function (e) {
    if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "s") {
      ...
      const config = typeof getVisibleDynamicFormTableConfig === "function"
        ...
        : null;
      if (config) {
        addDynamicFormTableRow(config.tbodyId, config.defaults || {});
        e.preventDefault();
      }
    } else if (e.key === "F8") {
      const config = typeof getVisibleDynamicFormTableConfig === "function"
        ? getVisibleDynamicFormTableConfig()
        : null;
      const tbody = config ? document.getElementById(config.tbodyId) : null;
      if (config && tbody) {
        const activeRow = tbody.contains(document.activeElement)
          ? document.activeElement.closest("tr")
          : tbody.querySelector("tr:last-child");
        if (activeRow) removeDynamicFormRow(activeRow.id, config.tbodyId);
        e.preventDefault();
      }
    }
  });
...
function getOrderTableRows(currentEl) {
  const tbody = currentEl?.closest?.('.dynamic-items-table tbody[id]');
  if (!tbody || typeof getDynamicFormTableConfig !== 'function') return null;
  const config = getDynamicFormTableConfig(tbody.id);
  if (!config) return null;
  return { tbody, rows: Array.from(tbody.querySelectorAll('tr')), tbodyId: tbody.id, config };
}
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'F1') {
      ...
      if (idx < fields.length - 1) {
        ...
        if (info) {
          addDynamicFormTableRow(info.tbodyId, info.config.defaults || {});
        }
      }
      return;
    }
    ...
    const info = getOrderTableRows(el);
    if (!info) return;
    const { rows, tbodyId, config } = info;
    ...
    if (e.key === 'Tab' && !e.shiftKey) {
      ...
      if (rowIdx < rows.length - 1) {
        ...
        addDynamicFormTableRow(tbodyId, config.defaults || {});
      }
      return;
    }
...
function closeMobileSidebar() {
  ...
  if (sidebar) sidebar.setAttribute('aria-hidden', isMobileSidebarView() ? 'true' : 'false');
...
window.addEventListener('resize', function () {
  if (!isMobileSidebarView()) {
    ...
    var sidebar = document.querySelector('.sidebar');
    if (sidebar && !sidebar.classList.contains('mobile-open')) sidebar.setAttribute('aria-hidden', 'true');
  }
});
