...
// Convert Excel Serial Date / Date / string → YYYY-MM-DD (local-safe, no UTC day shift).
function excelDateToISOString(serial) {
    ...
    if (serial instanceof Date) {
        if (Number.isNaN(serial.getTime())) {
            return excelDateToISOString("");
        }
        // SheetJS Date values are typically UTC midnight for the calendar day.
    ...
    if (typeof serial === "string") {
        ...
        if (!Number.isNaN(parsed.getTime())) {
            return excelDateToISOString(parsed);
        }
        return trimmed.split("T")[0];
    }
    if (typeof serial === "number" && Number.isFinite(serial)) {
        // Excel serial → UTC midnight of that calendar day
        const date = new Date(Math.round((serial - 25569) * 86400 * 1000));
        if (Number.isNaN(date.getTime())) return excelDateToISOString("");
