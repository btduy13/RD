...
function initApp() {
  ...
  // Tự động cập nhật đơn giá từ file S06-DNN trong thư mục excel/ (nếu có)
  setTimeout(() => {
    if (typeof autoApplyS06Prices === "function") {
      autoApplyS06Prices();
    }
  }, 3000); // Chờ 3 giây sau khởi động để app ổn định trước
}
...
async function autoIntegrateSoChiTietMuaHangExcel(force = false) {
  ...
  try {
    ...
    if (rows.length < 3) {
      console.warn("excel/SO_CHI_TIET_MUA_HANG_THEO_MA_QUY_CACH.xlsx is empty.");
      return;
    }
...
// ==========================================================================
// TỰ ĐỘNG CẬP NHẬT ĐƠN GIÁ TỪ FILE S06-DNN KHI KHỞI ĐỘNG
// ==========================================================================
/**
 * Chạy sau khi loadState() hoàn tất.
 * Tự tìm file S06 hoặc CSV trong thư mục excel/, đọc bằng Node.js qua
 * electronAPI.readExcelFile (không cần người dùng thao tác gì), bóc tách
 * đơn giá và cập nhật state.products cho các mặt hàng trùng mã.
 *
 * Ưu tiên đọc file theo thứ tự:
 *   1. gia_moi_tong_hop.csv  (file CSV đã xuất sẵn — đọc nhanh nhất)
 *   2. S06_DNN_*.xls / S06_DNN_*.xlsx  (file gốc MISA)
 */
async function autoApplyS06Prices() {
  if (!window.electronAPI || !window.electronAPI.readExcelFile) return;
  if (!state.products || state.products.length === 0) return;

  try {
    // Lấy danh sách file trong thư mục excel/
    let fileList = [];
    if (window.electronAPI.listExcelDir) {
      const res = await window.electronAPI.listExcelDir();
      if (res && res.ok) fileList = res.files || [];
    }

    // Xác định file cần đọc (ưu tiên CSV → XLS/XLSX S06)
    let targetFile = null;
    if (fileList.includes("gia_moi_tong_hop.csv")) {
      targetFile = "gia_moi_tong_hop.csv";
    } else {
      targetFile = fileList.find(f =>
        f.toLowerCase().startsWith("s06") && (f.endsWith(".xls") || f.endsWith(".xlsx"))
      ) || null;
    }

    if (!targetFile) return; // Không có file nào phù hợp

    // Đọc file qua Electron IPC (Node.js fs)
    const result = await window.electronAPI.readExcelFile(targetFile);
    ...
    if (targetFile.endsWith(".csv")) {
      ...
      // ── XLS/XLSX gốc MISA S06 ────────────────────────────────────────────
      if (typeof XLSX === "undefined") return;
      const wb = XLSX.read(bytes, { type: "array" });
      const ws = wb.Sheets[wb.SheetNames[0]];
      const rows = XLSX.utils.sheet_to_json(ws, { header: 1, defval: "" });
      let maHang = "", lastDonGia = 0;
      for (let r = 0; r < rows.length; r++) {
        const row = rows[r];
        const c0 = String(row[0] || "").trim();
        const c7 = String(row[7] || "").trim();
        if (c0.includes("hàng:") && c0.includes("ã")) {
          maHang = String(row[3] || "").trim().toUpperCase();
          lastDonGia = 0; continue;
        }
        if (c7.includes("đầu kỳ") && maHang) {
          const dg = parseFloat(row[11]) || 0;
          if (dg > 0) lastDonGia = dg; continue;
        }
        if (maHang && c0 && !["Cộng","Tổng cộng","MISA","CÔNG TY","SỔ","Từ ngày","Mã kho","Tên kho"].some(k => c0.includes(k))) {
          const dg = parseFloat(row[11]) || 0;
          if (dg > 0) lastDonGia = dg;
        }
        if (c0.includes("Cộng") && !c0.includes("Tổng") && maHang) {
          ...
          const tonTt = parseFloat(row[24]) || 0;
          let dg = lastDonGia;
          if (tonSl !== 0) dg = Math.round(tonTt / tonSl);
          if (dg > 0) priceMap[maHang] = dg;
          maHang = ""; lastDonGia = 0;
        }
      }
    }
    ...
    // Áp đơn giá vào state.products
    let updatedCount = 0;
    state.products.forEach(prod => {
      const key = prod.id.toUpperCase();
      if (priceMap.hasOwnProperty(key)) {
        const newPrice = priceMap[key];
        if (newPrice !== prod.avgCost) {
          prod.avgCost     = newPrice;
          prod.initialCost = newPrice;
          prod.totalValue  = (prod.stock || 0) * newPrice;
          updatedCount++;
    ...
    if (updatedCount > 0) {
      saveState();
      console.log(`[S06 Auto] Đã cập nhật đơn giá cho ${updatedCount} mặt hàng từ "${targetFile}"`);
      // Thông báo nhỏ ở góc màn hình (không làm phiền người dùng)
      setTimeout(() => {
        showToast(`🏷️ Đã tự cập nhật đơn giá ${updatedCount} mặt hàng từ file S06`, "info");
      }, 2500);
    }

  } catch (err) {
    console.warn("[S06 Auto] Bỏ qua lỗi khi tự cập nhật giá:", err.message);
  }
}

window.autoApplyS06Prices = autoApplyS06Prices;
