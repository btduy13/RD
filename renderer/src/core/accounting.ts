import type { AppState, Product } from "@/types/app-state";
...
export function getInventoryValueAt(state: AppState, toDate = ""): number {
  if (!toDate) {
    return (state.products || []).reduce((sum, p) => sum + (p.totalValue || 0), 0);
  }
  ...
  const chronologicalVouchers = [...(state.vouchers || [])].sort((a, b) =>
    (a.date || "").localeCompare(b.date || "")
  );
  for (const v of chronologicalVouchers) {
    if (v.date > toDate || !v.items) continue;
    const seen = new Set<string>();
    for (const item of v.items) {
      const pId = item.productId;
      if (!pId || productStocks[pId] === undefined || seen.has(pId)) continue;
      seen.add(pId);
      if (v.type === "purchase") {
        productStocks[pId] += item.qty || 0;
        productValues[pId] += item.amount || 0;
      } else if (v.type === "sales") {
        productStocks[pId] -= item.qty || 0;
        productValues[pId] -= (item as { cogsAmount?: number }).cogsAmount || 0;
      }
    }
  }
  return (state.products || []).reduce((sum, p) => sum + (productValues[p.id] || 0), 0);
}


export function recalculateAccounting(state: AppState, defaultProducts: Product[] = []): void {
  ...
  const originalProducts = defaultProducts;
...
function rebalanceEquity(state: AppState): void {
  let debitSum = 0;
  let creditSum = 0;
...
export function getAccountBalance(state: AppState, acctCode: string, toDate = ""): number {
