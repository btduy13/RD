...
// ==========================================================================
// @ts-nocheck — faithful port of untyped sync-v2.js; public API is typed below.
...
function getStoredLastPulledCloudTs() {
  const stateTs = Number(getBridgeState()?._lastPulledCloudTs ?? 0);
...
function getLegacyStartupCheckpointTs() {
  const currentState = getBridgeState();
  const stateTs = Number(originalStateLastModified || (currentState && currentState._lastModified) || 0);
...
function syncV2ApplyPushToLastSyncState(pushedEntityRows, pushTs) {
  lastSyncState = lastSyncState || lastSyncState;
  if (!lastSyncState) {
...
function syncV2MergeMetadata(localState, cloudState) {
  ...
  const localTs = Number(localState._lastModified) || Number(originalStateLastModified || localMeta._lastModified) || 0;
...
async function persistStateCacheAfterCloudPull(cacheState = getBridgeState()) {
  try {
    ...
    if (window.electronAPI && typeof window.electronAPI.writeStateFile === "function") {
      ...
      if (result && result.ok) syncBridge?.onStateSaved?.(); else if (result && !result.ok) {
...
async function syncV2PersistPullDeltaToCache(mergedState, changedIdsByEntity = null) {
  ...
  try {
    ...
    SYNC_V2_ENTITY_DEFS.forEach(def => {
      const snapshotMap = lastSavedStateSnapshot[def.stateKey];
      if (!(snapshotMap instanceof Map)) throw new Error(`lastSavedStateSnapshot.${def.stateKey} is not a Map`);
    ...
    metadataKeys.forEach(key => {
      ...
      const prevValStr = JSON.stringify(lastSavedStateSnapshot[key] !== undefined ? lastSavedStateSnapshot[key] : null);
    ...
    Object.keys(delta.metadata).forEach(key => {
      lastSavedStateSnapshot[key] = JSON.parse(delta.metadata[key]);
    });
    SYNC_V2_ENTITY_DEFS.forEach(def => {
      delta[def.stateKey].upsert.forEach(item => {
        lastSavedStateSnapshot[def.stateKey].set(item.id, JSON.parse(JSON.stringify(item)));
      });
      delta[def.stateKey].deleteIds.forEach(id => {
        lastSavedStateSnapshot[def.stateKey].delete(id);
      });
    });
...
export async function flushDeferredCloudSync() {
  ...
  try {
    ...
    if (options.forceFull || checkpoint === 0) {
      ...
      if (rows.length === 0 && options.retryFullIfNoChanges) {
        ...
        // (initApp sets lastSyncState during startup) - same pattern as computeDelta.
        lastSyncState = lastSyncState || lastSyncState;
    ...
    console.error("[CloudSyncV2] CloudSyncV2.pull:", err.message, err);
...
async function pullFromCloudOnStartup() {
  ...
  const hasBaseSnapshot = !!(lastSyncState || lastSyncState);
  ...
  try {
    ...
    console.error("[CloudSyncV2] CloudSyncV2.startup:", err.message, err);
...
export async function pushToCloud() {
  ...
  try {
    ...
    const afterPush = getBridgeState();
    if (tombstoneRows.length > 0) {
      setBridgeState({ ...afterPush, deletedIds: [], deletedCloudKeys: [], _cloudWatermark: pushTs });
    } else {
      setBridgeState({ ...afterPush, _cloudWatermark: pushTs });
    }
    ...
    console.error("[CloudSyncV2] CloudSyncV2.push:", err.message, err);
...
export function initCloudSync() {
  if (readCloudSyncSettings().enabled === false) {
  ...
  if (!readCloudSyncSettings().supabaseUrl || !readCloudSyncSettings().supabaseAnonKey) {
    cloudSyncActive = false;
    isStartupPullCompleted = true;
...
async function startSupabaseClient() {
  try {
    ...
    console.error("[CloudSyncV2] CloudSyncV2.init:", err.message, err);
    cloudSyncActive = false;
    isStartupPullCompleted = true;
    stopCloudMetadataPolling();
    stopRealtimeReconnect();
...
export function forcePushToCloud() {
  if (!cloudSyncActive || !supabaseClient) {
    bridgeToast("Ứng dụng chưa kết nối đám mây!", "danger");
    return;
  }
  if (isVoucherEntryModalOpen()) {
    bridgeToast("Hãy lưu hoặc đóng phiếu đang nhập trước khi đẩy cloud.", "warning");
    return;
  }
  if (appConfirm("Bạn có chắc muốn đẩy dữ liệu cục bộ lên cloud?")) {
    patchBridgeState({ _lastModified: Date.now() });
    pushToCloud().then(() => bridgeToast("Đã đẩy dữ liệu lên cloud.", "success"));
  }
}
...
export function forcePullFromCloud() {
  ...
  pullAndMergeFromCloud({ reason: "manual-full", forceFull: true, force: true })
    .then(() => bridgeToast("Đã tải và hợp nhất dữ liệu cloud.", "success"))
    .catch(err => bridgeToast("Lỗi tải cloud: " + err.message, "danger"));
}

export async function pullFromCloud(options: { forceFull?: boolean } = {}): Promise<void> {
  if (!cloudSyncActive || !supabaseClient) return;
  await pullAndMergeFromCloud({
    reason: options.forceFull ? "manual-full" : "manual",
    forceFull: !!options.forceFull,
    force: true,
    retryFullIfNoChanges: !options.forceFull,
  });
}

export function manualIncrementalSync() {
  if (!cloudSyncActive || !supabaseClient) {
    bridgeToast("Ứng dụng chưa kết nối đám mây!", "danger");
    return;
  }
  pullAndMergeFromCloud({ reason: "manual", retryFullIfNoChanges: true, force: true })
    .then(() => bridgeToast("Đồng bộ cloud thành công.", "success"))
    .catch(err => bridgeToast("Lỗi đồng bộ: " + err.message, "danger"));
}
...
function getMaxLocalVoucherSequence(prefix, excludeId = null) {
  let maxNum = 0;
  (getBridgeState().vouchers || []).forEach(v => {
...
export async function getCloudSafeVoucherId(options = {}) {
...
export async function ensureCloudSafeVoucherIdForSave(options = {}) {
...
export {
  ...
  updateCloudSyncBadge as setSyncBadgeCallback,
