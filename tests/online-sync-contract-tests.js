...
const reliabilityMigration = fs.readFileSync(path.join(root, 'supabase_online_v5_sync_reliability_migration.sql'), 'utf8');
...
assert.match(syncSource, /p_expected_sync_version: transactionExpectedVersion/);
...
assert.match(reliabilityMigration, /grant select\s*\(\s*workspace_id\s*,\s*id\s*,\s*sync_version\s*,\s*updated_by\s*\)\s*on public\.rd_accounting_data to anon, authenticated/i);
assert.match(reliabilityMigration, /using\s*\(\s*id\s*=\s*'sync_signal'\s*\)/i, 'Realtime metadata access must be restricted to the compact signal row');
assert.match(reliabilityMigration, /create or replace function public\.rd_rows_by_ids/i, 'the release migration must install the missing stale-tombstone RPC');
assert.match(reliabilityMigration, /text_pattern_ops/i, 'voucher prefix lookup must have an indexable prefix path');
assert.doesNotMatch(reliabilityMigration, /left\s*\(\s*d\.id\s*,\s*length\s*\(\s*p_prefix\s*\)\s*\)/i, 'voucher prefix lookup must not scan every cloud row');
