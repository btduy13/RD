...
-- Keep the existing id primary key during the rolling upgrade so an already
-- running legacy client can still use PostgREST upsert until it restarts and
-- switches to the V3 RPC protocol. The workspace-scoped unique key is used by
-- every V3 transaction. A future multi-workspace rollout can promote this
-- unique key to the primary key after all legacy clients have been retired.
do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conrelid = 'public.rd_accounting_data'::regclass
      and conname = 'rd_accounting_data_workspace_id_id_key'
  ) then
    alter table public.rd_accounting_data
      add constraint rd_accounting_data_workspace_id_id_key unique(workspace_id, id);
  end if;
end $$;
...
begin
  select sync_version into v_current from public.rd_workspaces where id=p_workspace_id for update;
  delete from public.rd_accounting_data
    where workspace_id=p_workspace_id
      and id like 'lock\_%' escape '\'
      and last_modified < (extract(epoch from clock_timestamp())*1000)::bigint - 30*60*1000;
  v_next:=v_current+1;
...
alter table public.rd_workspaces enable row level security;
